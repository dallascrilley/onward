...
# File format: <count>:<timestamp>

# Returns throttle file path for a session ID
throttle_file_for_session() {
    local session_id="$1"
    echo "/tmp/.claude-continue-throttle-${session_id//\//_}"
}
...
throttle_read() {
    ...
    CONTINUE_COUNT="${throttle_data%:*}"
    LAST_CONTINUE_TIME="${throttle_data#*:}"
}
...
throttle_write() {
    ...
    local count="$2"
    local timestamp="$3"
    echo "$count:$timestamp" > "$throttle_file"
}
